...
function AppContent() {
  ...
  const [feedbackEmoji, setFeedbackEmoji] = React.useState<{ emoji: string; text?: string } | null>(null);
  const feedbackTimeoutRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);

  React.useEffect(() => {
    const handleEmoji = (event: Event) => {
      const customEvent = event as CustomEvent<{ emoji: string; text?: string }>;
      if (!customEvent.detail?.emoji) return;

      setFeedbackEmoji(customEvent.detail);
    ...
    window.addEventListener('tabbie:emoji', handleEmoji);

    return () => {
      window.removeEventListener('tabbie:emoji', handleEmoji);
      if (feedbackTimeoutRef.current) {
        clearTimeout(feedbackTimeoutRef.current);
      }
    };
  ...
  return (
    <>
      ...
      {feedbackEmoji && (
        <div className="fixed top-6 right-6 z-[9999] flex items-center gap-3 rounded-2xl border border-border bg-card px-5 py-4 shadow-2xl">
          <div className="text-5xl leading-none">{feedbackEmoji.emoji}</div>
          {feedbackEmoji.text && (
            <div className="text-sm font-semibold text-foreground">{feedbackEmoji.text}</div>
          )}
        </div>
      )}
