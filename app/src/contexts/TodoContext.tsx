...
const emitTabbieEvent = (event: TabbieEventName, payload?: Record<string, unknown>) => {
  window.dispatchEvent(
    new CustomEvent('tabbie:event', {
      detail: {
        event,
        payload: payload || {},
...
export const TodoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const toggleTaskComplete = (taskId: string) => {
    ...
    if (!task.completed) {
      ...
      emitTabbieEvent('task_completed', {
        taskId: task.id,
        title: task.title,
        categoryId: task.categoryId,
        totalPomodoros: completedPomodoros,
...
const session: PomodoroSession = {
  ...
  type: 'work',
    ...
    emitTabbieEvent('pomodoro_started', {
      taskId: task.id,
      title: task.title,
  ...
  const pausePomodoro = () => {
    emitTabbieEvent('pomodoro_paused', {
  ...
  const resumePomodoro = () => {
    emitTabbieEvent('pomodoro_resumed', {
      taskId: currentTaskId,
    });
  ...
  const stopPomodoro = async () => {
    ...
    emitTabbieEvent('pomodoro_stopped', {
      taskId: pomodoroTimer.currentSession?.taskId || currentTaskId,
      wasOvertime: pomodoroTimer.timeLeft < 0,
    });
  ...
  const completePomodoro = () => {
    if (pomodoroTimer.currentSession) {
      ...
      emitTabbieEvent(pomodoroTimer.sessionType === 'work' ? 'pomodoro_completed' : 'break_completed', {
        taskId: currentTask?.id,
        title: currentTask?.title,
        sessionType: pomodoroTimer.sessionType,
      });
